...
import { MARGIN_0, OPACITY_15, TRANSPARENT, WHITE, RICH_BLACK, DULLS_BACKGROUND_COLOR } from '@platformatic/ui-components/src/components/constants'
  ...
  return (
    <div className={`${commonStyles.tinyFlexBlock} ${commonStyles.fullWidth}`}>
      <p className={`${typographyStyles.desktopBodySmall} ${typographyStyles.textWhite} ${commonStyles.fullWidth}`}>
        <span className={`${typographyStyles.opacity70}`}>You are about to edit the path of your application </span>{name}<span className={`${typographyStyles.opacity70}`}>.</span>
      </p>
      ...
      <form onSubmit={handleSubmit} className={`${commonStyles.tinyFlexBlock} ${commonStyles.fullWidth}`}>
        ...
        <div className={`${commonStyles.smallFlexRow} ${commonStyles.fullWidth} ${commonStyles.justifyBetween}`}>
          ...
          <Button
            ...
            textClass={typographyStyles.desktopButtonSmall}
            paddingClass={commonStyles.smallButtonPadding}
            label={updating ? 'Updating...' : 'Confirm edit'}
            onClick={handleSubmit}
            color={RICH_BLACK}
            backgroundColor={WHITE}
            hoverEffect={DULLS_BACKGROUND_COLOR}
            disabled={!validForm || updating || originalPath === form.path}
            bordered={false}
          />
        </div>
      </form>
