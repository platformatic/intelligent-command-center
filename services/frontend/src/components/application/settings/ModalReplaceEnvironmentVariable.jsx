...
function ModalReplaceEnvironmentVariable ({
  name = '-',
  onClickCancel = () => {},
  onClickConfirm = () => {}
}) {
    ...
    <div className={`${commonStyles.mediumFlexBlock} ${commonStyles.fullWidth}`}>
      <p className={`${typographyStyles.desktopBody} ${typographyStyles.textWhite} ${commonStyles.fullWidth}`}>
        <span className={`${typographyStyles.opacity70}`}>
          You are about to replace the existing variables {name}.
...
ModalReplaceEnvironmentVariable.propTypes = {
  /**
   * name
   */
...
export default ModalReplaceEnvironmentVariable
